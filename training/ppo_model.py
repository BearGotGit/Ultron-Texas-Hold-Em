...
class PokerPPODiscreteModel(nn.Module):
    """
    Discrete-action PPO model variant.
    ...
    This class mirrors the shared embedding + trunk from `PokerPPOModel`
    but exposes a categorical `action_head` instead of fold/beta heads.
    """
    ...
    def __init__(
        ...
        # reuse the same submodules (structure copied from PokerPPOModel)
        self.card_embedding = CardEmbedding(card_embed_dim)
        self.hand_embedding = nn.Sequential(
            nn.Linear(NUM_HAND_FEATURES, 32),
            nn.ReLU(),
        ...
        numeric_input_dim = MAX_PLAYERS * FEATURES_PER_PLAYER + GLOBAL_NUMERIC_FEATURES
        self.numeric_embedding = nn.Sequential(
            nn.Linear(numeric_input_dim, 64),
            nn.ReLU(),
            nn.Linear(64, 64),
        ...
        self.card_features = NUM_CARD_SLOTS * card_embed_dim
        self.hand_embed_features = 32
        self.numeric_embed_features = 64

        self.combined_features = (
            self.card_features + self.hand_embed_features + self.numeric_embed_features
        )
        ...
        # Discrete action head (logits for each action)
    ...
    def _init_weights(self):
        ...
        # smaller init for action head final layer to avoid extreme logits
        action_final = self.action_head[-1]
        nn.init.orthogonal_(action_final.weight, gain=0.01)
        nn.init.zeros_(action_final.bias)

        value_final = self.value_head[-1]
        nn.init.orthogonal_(value_final.weight, gain=1.0)
        nn.init.zeros_(value_final.bias)

    def _embed_cards(self, obs: torch.Tensor) -> torch.Tensor:
        batch_size = obs.shape[0]
        card_obs_dim = NUM_CARD_SLOTS * CARD_ENCODING_DIM
        card_obs = obs[:, :card_obs_dim]
        card_obs = card_obs.view(batch_size, NUM_CARD_SLOTS, CARD_ENCODING_DIM)
        card_embeddings = []
        for i in range(NUM_CARD_SLOTS):
            card_emb = self.card_embedding(card_obs[:, i, :])
            card_embeddings.append(card_emb)
        return torch.cat(card_embeddings, dim=1)
    ...
    def _extract_hand_features(self, obs: torch.Tensor) -> torch.Tensor:
        card_obs_dim = NUM_CARD_SLOTS * CARD_ENCODING_DIM
        hand_start = card_obs_dim
        hand_end = hand_start + NUM_HAND_FEATURES
        return obs[:, hand_start:hand_end]

    def _extract_numeric_features(self, obs: torch.Tensor) -> torch.Tensor:
        card_obs_dim = NUM_CARD_SLOTS * CARD_ENCODING_DIM
        numeric_start = card_obs_dim + NUM_HAND_FEATURES
        return obs[:, numeric_start:]

    def forward(self, obs: torch.Tensor):
        card_features = self._embed_cards(obs)
        hand_raw = self._extract_hand_features(obs)
        hand_features = self.hand_embedding(hand_raw)
        numeric_raw = self._extract_numeric_features(obs)
        numeric_features = self.numeric_embedding(numeric_raw)
    ...
    def evaluate_actions(self, obs: torch.Tensor, actions: torch.Tensor):
        logits, value = self.forward(obs)
        dist = Categorical(logits=logits)
        # actions expected shape (batch, 1) -> squeeze
        a = actions.squeeze(-1).long()
...
class RLPokerDiscretePlayer:
    """
    Wrapper that returns a discrete action id from the discrete PPO model.
    """
    def __init__(self, player_id: str, model: PokerPPODiscreteModel, device: torch.device = torch.device("cpu"), deterministic: bool = False):
        self.id = player_id
        self.model = model
        self.device = device
        self.deterministic = deterministic

    def get_action_from_obs(self, obs: torch.Tensor) -> int:
        """
        Returns an integer action id in [0..5].
        """
        self.model.eval()
        with torch.no_grad():
            obs = obs.unsqueeze(0).to(self.device)
            action, _, _, _ = self.model.get_action_and_value(obs, deterministic=self.deterministic)
            action = action.squeeze(0).cpu().numpy()
        return int(action[0])
